...
/* Table schema:
CREATE TABLE kv_store_9f7fc7bb (
  key TEXT NOT NULL PRIMARY KEY,
  value JSONB NOT NULL
);
*/

// View at https://supabase.com/dashboard/project/vbzmelpvugyixagfiftu/database/tables
...
export const del = async (key: string): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_9f7fc7bb").delete().eq("key", key);
  if (error) {
    throw new Error(error.message);
  }
};
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_9f7fc7bb").delete().in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
};
